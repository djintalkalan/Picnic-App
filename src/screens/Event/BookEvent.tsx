...
const BookEvent: FC = (props: any) => {
    ...
    const onSubmit = useCallback(() => handleSubmit(data => {
        if (event?.is_free_event)
            ...
            buttonText: Language.pay + ' ' + getSymbol(event?.event_currency) + round(parseInt(noOfTickets) * (event?.event_fees + event?.event_tax_amount), 2),
            buttonStyle: { width: '100%' }
        })
    })(), [event, noOfTickets, isPayByPaypal])
                ...
                {!event?.is_free_event ?
                    <>
                        <View style={{ height: 1, width: '100%', backgroundColor: '#DBDBDB', alignSelf: 'center', marginVertical: scaler(16) }} /><Text style={{ marginLeft: scaler(8), fontSize: scaler(14), fontWeight: '500' }}>
                            ...
                            {noOfTickets ? getSymbol(event?.event_currency) + parseInt(noOfTickets) * event?.event_tax_amount : getSymbol(event?.event_currency) + 0}
                        </Text>
                    ...
                    {noOfTickets ?
                        <Button
                            title={event?.is_free_event ? Language.book_ticket
                                : Language.pay + ' ' + getSymbol(event?.event_currency) + round(parseInt(noOfTickets) * (event?.event_fees + event?.event_tax_amount), 2)}
                            onPress={onSubmit}
