...
const Home: FC = () => {
  ...
  const [currentLocation] = useDatabase<ILocation>(
    'currentLocation',
    defaultLocation,
  );
  const [selectedLocation, setSelectedLocation] = useDatabase<ILocation>(
    'selectedLocation',
    currentLocation,
  );

  const [profileImage, setProfileImage] = useState();
  const isFabTransparent =
    (currentTabIndex && !eventLength) || (!currentTabIndex && !groupLength);
  return (
      ...
      <View style={styles.headerContainer}>
        <TouchableOpacity
          ...
          <Text numberOfLines={1} style={styles.locationText}>
            {selectedLocation?.address?.main_text}
          </Text>
          <Octicons
            style={{marginLeft: scaler(6)}}
            name={'chevron-down'}
            size={scaler(18)}
          />
        </TouchableOpacity>
